...
while True:
    ...
    if raw == "A":
        ...
        # sort the full status list
        update_1 = sorted(update_1, key=lambda x: int(x[0]))
        update_2 = sorted(update_2, key=lambda x: int(x[0]))



        # print the sorted variable
        for line in update_1:
            obj = pack_table.search(line[0])
            if to_minutes(time) < 140:
                if obj.id == "9":
                    obj.address = "300 State St"
            print("Package " + line[0] + " is on truck 1 and " + line[1] + " and its delivery time is " + line[2] + " to " + obj.address + " by " + obj.deadline)
        for line in update_2:
            obj = pack_table.search(line[0])
            if to_minutes(time) < 140:
                if obj.id == "9":
                    obj.address = "300 State St"
            print("Package " + line[0] + " is on truck 2 and " + line[1] + " and its delivery time is " + line[2] + " to " + obj.address + " by " + obj.deadline)
        print("Total Mileage Travelled " + str(path_1[-1][1] + path_2[-1][1]))
    elif raw == "Q":
        break
